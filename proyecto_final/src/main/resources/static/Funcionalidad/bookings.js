...
async function cargarGimnasios() {
    try {
        const respuesta = await fetch('/api/bookings/gyms');
        if (respuesta.ok) {
            gimnasios = await respuesta.json();
            renderizarGimnasios();
        }
    } catch (error) {
            ...
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <div class="flex items-center gap-3 mb-2">
                        <h3 class="text-2xl font-medium">${gimnasio.nombre}</h3>
                         <div class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold bg-white border-orange-200 text-orange-600 shadow-sm">
                             <i data-lucide="star" class="w-3 h-3 mr-1 fill-orange-500 text-orange-500"></i> ${gimnasio.valoracion}
                        </div>
                    </div>
                    <div class="flex items-center text-gray-600">
                        <i data-lucide="map-pin" class="w-5 h-5 mr-2 text-orange-500"></i>
                         <span class="text-lg">${gimnasio.ubicacion}</span>
                    </div>
                </div>
                 ${estaSeleccionado ? `<div class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-gradient-to-r from-orange-500 to-orange-600 text-white border-0 shadow-md"> <i data-lucide="check-circle-2" class="w-3.5 h-3.5 mr-1"></i> Seleccionado</div>` : ''}
            </div>
        `;
    ...
    gimnasio.salas.forEach(sala => {
        ...
        el.innerHTML = `
            <div class="flex items-center gap-4 mb-4">
                ...
                <div class="flex-1">
                    <h3 class="text-xl mb-1 font-medium">${sala.nombre}</h3>
                    <div class="flex items-center gap-1 text-sm text-gray-600">
                         <i data-lucide="users" class="w-4 h-4"></i> <span>Hasta ${sala.capacidad}</span>
                    </div>
                </div>
            </div>
            <div class="pt-4 border-t border-gray-200 flex justify-between">
                <span class="text-sm text-gray-600">Precio/hora</span>
                <span class="text-2xl font-bold bg-gradient-to-r from-orange-600 to-orange-500 bg-clip-text text-transparent">€${sala.precio}</span>
            </div>
        `;
        el.onclick = () => {
...
function actualizarResumen() {
    ...
    if (elNombreGimnasio) elNombreGimnasio.innerText = idGimnasioSeleccionado ? gimnasios.find(g => g.id === idGimnasioSeleccionado).nombre : 'Selecciona un gimnasio';

    let precio = 0;
    if (idGimnasioSeleccionado && idSalaSeleccionada) {
        const sala = gimnasios.find(g => g.id === idGimnasioSeleccionado).salas.find(r => r.id === idSalaSeleccionada);
        if (sala) {
            if (elNombreSala) elNombreSala.innerText = sala.nombre;
            precio = sala.precio;
        }
    } else {
...
function configurarOyentesReserva() {
    ...
    if (btn) {
        btn.onclick = async () => {
            const usuarioJSON = localStorage.getItem('gymCoreUser');
            if (!usuarioJSON) {
            ...
            const datosReserva = {
                ...
                salaId: idSalaSeleccionada,
                horario: tiempoSeleccionado,
                precio: sala.precio
            };

            try {
                const respuesta = await fetch('/api/bookings/reserve', {
                    method: 'POST',
